"""
proxy.py - Proxy Server (Node 2, Port 8080)
Jaringan Komputer - Tugas Besar: Client-Proxy-Server Architecture

Fitur:
  - Multithreading (satu thread per koneksi client)
  - Caching file lokal (cache/<path>.cache) dengan threading.Lock agar aman dari race condition
  - Failover otomatis: Server Utama (8000) → Server Cadangan (8001)
  - Log lengkap: IP Client, URL Path, Cache HIT/MISS, server yang merespons, response time

Penggunaan:
  python proxy.py --main_ip <IP_NODE3> --backup_ip <IP_NODE4>

Contoh:
  python proxy.py --main_ip 192.168.1.10 --backup_ip 192.168.1.11
"""
